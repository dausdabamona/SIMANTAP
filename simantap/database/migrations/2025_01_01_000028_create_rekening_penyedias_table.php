<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        Schema::create('rekening_penyedias', function (Blueprint $table) {
            $table->id();
            $table->foreignId('penyedia_id')->constrained('penyedia_makan')->cascadeOnDelete();
            $table->string('label', 100)->nullable();
            $table->string('bank', 100);
            $table->string('nomor_rekening', 50)->unique();
            $table->string('nama_pemilik', 150);
            $table->boolean('is_active')->default(true);
            $table->boolean('is_default')->default(false);
            $table->date('berlaku_mulai')->nullable();
            $table->date('berlaku_sampai')->nullable();
            $table->text('alasan_perubahan')->nullable();
            $table->foreignId('dibuat_by')->constrained('users')->restrictOnDelete();
            $table->timestamps();
            $table->softDeletes();

            $table->index(['penyedia_id', 'is_default']);
            $table->index(['penyedia_id', 'is_active']);
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('rekening_penyedias');
    }
};
